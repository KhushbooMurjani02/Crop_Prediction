...
@app.route('/grading/<user>',methods=['GET'])
def hello_user(user):
    model = GaussianNB()
    df2 = pd.read_csv('G:/BE PROJECT(GROUP NO 16)/mushrooms.csv')
    la=[]
    a=[]
    b=[]
    c=[]
    ...
    if predicted[0]=='0':
        return jsonify({"paramkey":"B"})
    else:
        return jsonify({"paramkey":"A"})

if __name__ == '__main__':
    app.run(host='192.168.10.104', port=8000)